...
void init() {
	...
	fin = fopen((inPath + "amazon_book_relation2id.txt").c_str(), "r");
	tmp = fscanf(fin, "%d", &relationTotal);
	fclose(fin);

	fin = fopen((inPath + "amazon_book_entity2id.txt").c_str(), "r");
	...
	fin = fopen((inPath + "amazon_book_triple2id.txt").c_str(), "r");

    ...
    def __init__(self, params):
        ...
        if self.has_attn:
            if params.use_kge_embeddings:
                ...
                self.attn_rel_emb = nn.Embedding.from_pretrained(torch.FloatTensor(node_features[:self.num_rels]))
                if params.finetune_ebd:
                    self.attn_rel_emb.weight.requires_grad = True
                else:
                    self.attn_rel_emb.weight.requires_grad = False
            else:

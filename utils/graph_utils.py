...
def deserialize(data, train):
    data_tuple = pickle.loads(data)
    if train:
        keys = ('pos_nodes', 'neg_nodes', 'pos_n_label', 'neg_n_label')
    else:
        keys = ('pos_nodes', 'pos_n_label')
...
def ssp_multigraph_to_dgl(graph, n_feats=None):
    ...
    # Add edges
    for rel in tqdm(range(len(graph))):
    ...
    # make dgl graph，multigraph表示两个节点之间可以有多个边
...
def collate_dgl_train(samples):
    ...
    uid, iid, label, desc_of_item_hist, desc_of_item_neighbor, desc_of_neg_item_neighbor, \
        subgraphs, neg_subgraphs, _ids = map(list, zip(*samples))
    batched_graph_pos = dgl.batch(subgraphs)
    batched_graph_neg = dgl.batch(neg_subgraphs)
    return uid, iid, label, \
        torch.from_numpy(np.array(desc_of_item_hist)), \
        torch.from_numpy(np.array(desc_of_item_neighbor)), \
        torch.from_numpy(np.array(desc_of_neg_item_neighbor)), \
        batched_graph_pos, batched_graph_neg, torch.tensor(_ids)
        
def collate_dgl(samples):
    # The input `samples` is a list of pairs
    uid, iid, label, desc_of_item_hist, desc_of_item_neighbor, \
        subgraphs, _ids = map(list, zip(*samples))
    batched_graph_pos = dgl.batch(subgraphs)
    # batched_graph_neg = dgl.batch(neg_subgraphs)
    return uid, iid, label, \
        torch.from_numpy(np.array(desc_of_item_hist)), \
        torch.from_numpy(np.array(desc_of_item_neighbor)), \
        batched_graph_pos, torch.tensor(_ids)
